...
$sql = "SELECT Patient.FirstName, Patient.LastName, TreatmentType.Name AS n, PatientAdmission.AdmissionTime, TimeStamp FROM PatientTreatment JOIN TreatmentType ON PatientTreatment.TreatmentTypeId = TreatmentType.Id JOIN PatientAdmission ON PatientAdmission.Id = PatientTreatment.PatientAdmissionId JOIN Patient On Patient.Id = PatientAdmission.PatientId ORDER BY AdmissionTime DESC, TimeStamp ASC;
";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr><td> " . $row["FirstName"] . " </td><td> " . $row["LastName"] . "</td><td>" . $row["n"] . "</td><td>" . $row["AdmissionTime"] . "</td><td>" . $row["TimeStamp"] ."</td></tr>";
