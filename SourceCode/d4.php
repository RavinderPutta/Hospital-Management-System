...
$sql = "SELECT Diagnosis a, COUNT(Diagnosis) AS Occurences FROM (SELECT Id AS eID FROM Employee WHERE Id = " . $_POST["ddlIns"] . ") AS t1 JOIN (SELECT EmployeeId AS eID, PatientAdmissionId AS paID FROM PatientDoctor) AS t2 ON t1.eID = t2.eID JOIN (SELECT PatientAdmissionId AS paID, DiagnosisTypeId AS dtID FROM PatientDiagnosis) AS t3 ON t2.paID = t3.paID JOIN (SELECT Id AS dtID, Name AS Diagnosis FROM DiagnosisType) AS t4 ON t3.dtID = t4.dtID GROUP BY t4.Diagnosis ORDER BY t4.Diagnosis DESC; ";
