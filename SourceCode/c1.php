...
$sql = "SELECT ID, name, count(t1.paID) AS Occurences FROM (SELECT id as paID FROM PatientAdmission WHERE AdmissionTypeId = 2 AND DischargeTime IS NULL) AS t1 JOIN (SELECT DiagnosisTypeId AS dtID, PatientAdmissionId AS paID FROM PatientDiagnosis) AS t2 ON t1.paID = t2.paID JOIN DiagnosisType ON t2.dtID = DiagnosisType.Id GROUP BY id ORDER BY Occurences DESC;";
