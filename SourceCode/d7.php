...
FROM (SELECT t1.eID, COUNT(t2.paID) AS TreatmentCount 
FROM (SELECT EmployeeID AS eID, PatientAdmissionId as paID FROM PatientDoctor) AS t1
 JOIN (SELECT ID AS paID, DischargeTime FROM PatientAdmission) AS t2 ON t1.paID = t2.paID 
WHERE DischargeTime IS NULL
 GROUP BY t1.eID) AS CountTable
 JOIN Employee ON CountTable.eID = Employee.ID 
WHERE TreatmentCount = (SELECT COUNT(ID) AS AdmittedPatients FROM PatientAdmission WHERE DischargeTime IS NULL); ";
