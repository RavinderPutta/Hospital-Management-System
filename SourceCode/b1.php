...
$sql = "SELECT SQL_NO_CACHE * FROM Patient JOIN InsuranceProvider ON Patient.InsuranceProviderId = InsuranceProvider.Id";
...
if ($result->num_rows > 0) {
	...
	. $row	["EContactName"] . "</td><td>" . "(".substr($row["EContactNumber"], 0, 3).") ".substr($row["EContactNumber"], 3, 3)."-".substr($row["EContactNumber"],6) . "</td><td>"
	. $row	["InsuranceNumber"] . "</td><td>" . $row["Name"] . "</td></tr>";
    }
} else {
    echo "0 results";
}
