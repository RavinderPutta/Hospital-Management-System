...
$sql = "SELECT ID, name, count(t1.paID) AS Occurences FROM (SELECT id as paID FROM PatientAdmission WHERE AdmissionTypeId = 2 AND DischargeTime IS NULL) AS t1
 JOIN (SELECT TreatmentTypeId AS ttID, PatientAdmissionId AS paID FROM PatientTreatment) AS t2 ON t1.paID = t2.paID
 JOIN TreatmentType ON t2.ttID = TreatmentType.id GROUP BY id ORDER BY Occurences DESC; ";
